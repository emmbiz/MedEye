...
print("Training {} images.".format(train_generator.samples))
...
for i in range(15):
    ...
    color = 'red' if class_index == 0 else 'green' if class_index == 1 else 'black'
    plt.title(class_names[class_index], color=color)
...
# Print the full model including layers of the base model plus the dense layer
print(model.summary())

# Train the model including the validation dataset for validation
EPOCHS = 20
history = model.fit(
    train_generator,
...
model.save("mnet_xray_classifier.h5")
print("Model saved as", model_file_name)

# Convert Keras model to TF Lite format
converter = tf.lite.TFLiteConverter.from_keras_model(model)
tflite_float_model = converter.convert()
open("mnet_xray_classifier.tflite", "wb").write(tflite_float_model)
...
# Get the class labels with the value 1
